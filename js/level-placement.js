(function attachLootLinguaLevelPlacement(root) {
  ...
  const LEVEL_PLACEMENT_VERSION = 2;
  ...
  function rankVersion(rank) {
    const version = Number(rank?.version);
    return Number.isSafeInteger(version) && version > 0 ? version : 1;
  }

  function rankSetSnapshot(ranks) {
    const published = (Array.isArray(ranks) ? ranks : [])
      .filter((rank) => rank?.status === 'published' && String(rank.rankId || '').trim())
      .slice()
      .sort((left, right) => schemaApi().comparePublishedRanks(left, right));
    const assessedRankIds = published.map((rank) => String(rank.rankId));
    const assessedRankVersions = Object.fromEntries(
      published.map((rank) => [String(rank.rankId), rankVersion(rank)])
    );
    const publishedRankSetHash = hashText(assessedRankIds
      .map((rankId) => `${rankId}:${assessedRankVersions[rankId]}`)
      .join('|')).toString(36);
    return { assessedRankIds, assessedRankVersions, publishedRankSetHash };
  }
  ...
  function mergeRankCoverage(previousHistory, testedRanks, allPublishedRanks) {
    const previousIds = new Set((previousHistory?.assessedRankIds ||
      previousHistory?.orderedRankIds || []).map(String));
    const versions = { ...(previousHistory?.assessedRankVersions || {}) };
    (Array.isArray(testedRanks) ? testedRanks : []).forEach((rank) => {
      const rankId = String(rank?.rankId || '');
      if (!rankId) return;
      previousIds.add(rankId);
      versions[rankId] = rankVersion(rank);
    });
    const ordered = (Array.isArray(allPublishedRanks) ? allPublishedRanks : [])
      .filter((rank) => previousIds.has(String(rank?.rankId || '')))
      .sort((left, right) => schemaApi().comparePublishedRanks(left, right))
      .map((rank) => String(rank.rankId));
    const remaining = [...previousIds].filter((rankId) => !ordered.includes(rankId)).sort();
    const assessedRankIds = [...ordered, ...remaining];
    const publishedRankSetHash = hashText(assessedRankIds
      .map((rankId) => `${rankId}:${Number(versions[rankId]) || 1}`)
      .join('|')).toString(36);
    return { assessedRankIds, assessedRankVersions: versions, publishedRankSetHash };
  }
  ...
  function buildLevelSample(input) {
    ...
    rankBundles.forEach((bundle, index) => {
      ...
      rankVersions[rankId] = rankVersion(bundle.rank);
    ...
    const snapshot = rankSetSnapshot(rankBundles.map((bundle) => bundle.rank));
    return {
      cefrLevel,
      assessmentSeed: seed,
      orderedRankIds: rankBundles.map((bundle) => String(bundle.rank.rankId)),
      rankVersions,
      publishedRankSetHash: snapshot.publishedRankSetHash,
      rankTitles,
      rankFirstGateIds,
      rankCoverage,
      selectedWords,
  ...
  function createSessionSeed(input) {
    ...
    const previousHistory = input?.previousHistory || null;
    const testedRankIds = sample.orderedRankIds.slice();
    const previousIds = new Set((previousHistory?.assessedRankIds ||
      previousHistory?.orderedRankIds || []).map(String));
    testedRankIds.forEach((rankId) => previousIds.add(String(rankId)));
    const assessedRankVersions = {
      ...(previousHistory?.assessedRankVersions || {}),
      ...(sample.rankVersions || {}),
    };
    const assessedRankIds = [
      ...sample.orderedRankIds.filter((rankId) => previousIds.has(String(rankId))),
      ...[...previousIds].filter((rankId) => !sample.orderedRankIds.includes(rankId)).sort(),
    ];
    return {
      assessmentId: id,
      ...
      assessmentMode: input?.assessmentMode === 'new-ranks' ? 'new-ranks' : 'full-level',
      previousAssessmentId: String(previousHistory?.assessmentId || ''),
      testedRankIds,
      assessedRankIds,
