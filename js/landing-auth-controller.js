...
export function authErrorMessage(error) {
  ...
  if (code === 'auth/email-already-in-use') {
    return 'يوجد حساب مرتبط بهذا البريد بالفعل.';
  }
  ...
  if (code === 'auth/network-request-failed') {
    return 'تعذر الاتصال الآن. جرّب مرة أخرى.';
  }
  if (code === 'auth/too-many-requests') {
    return 'تكررت المحاولات بسرعة. انتظر قليلًا ثم جرّب مرة أخرى.';
  }
  if (code === 'auth/user-disabled') {
    return 'هذا الحساب غير متاح حاليًا.';
  }
  if (code === 'auth/operation-not-allowed') {
    return 'تسجيل الدخول بالبريد غير متاح الآن.';
  }
...
export function validateAuthFields(mode, fields = {}) {
  ...
  if (mode !== AUTH_MODES.RESET) {
    ...
    else if (mode === AUTH_MODES.SIGNUP && password.length < 6) {
      errors.password = 'استخدم 6 أحرف على الأقل.';
    }
  }
...
export function createLandingAuthController({ gateway, onStateChange, navigateToApp }) {
  if (!gateway || typeof onStateChange !== 'function' || typeof navigateToApp !== 'function') {
    throw new TypeError('Landing Auth controller dependencies are required.');
  }
  ...
  async function initialize() {
    ...
    unsubscribe = gateway.observeAuth((user) => {
      emit({
        authResolved: true,
        user: user || null,
        busy: redirectPending || state.busy,
      });
    });
  ...
  async function signInWithGoogle() {
    if (state.busy) return false;
    if (state.user || gateway.currentUser?.()) {
      navigateToApp();
      return true;
    }
