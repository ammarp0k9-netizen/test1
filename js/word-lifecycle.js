(function attachLootLinguaWordLifecycle(root) {
  ...
  const PERSONAL_DICTIONARY_STATES = Object.freeze([
    'active',
  ...
  function getPersonalDictionaryState(word) {
    return word?.personalDictionaryState === 'moved-to-private-world'
      ? 'moved-to-private-world'
      : 'active';
  }

  function isMovedToPrivateWorld(word) {
    return getPersonalDictionaryState(word) === 'moved-to-private-world';
  }

  function isVisibleInPersonalDictionaryList(word) {
    return Boolean(word) && !isHiddenFromDictionary(word) && !isMovedToPrivateWorld(word);
  }

  function isVisibleInDictionaryList(word) {
    return isVisibleInPersonalDictionaryList(word);
  }
  ...
  function isEligibleForPersonalDictionaryQuiz(word) {
    return hasLearnableContent(word) && !isMovedToPrivateWorld(word);
  }

  function isEligibleForPrivateWorldQuiz(word, worldId) {
    if (!hasLearnableContent(word)) return false;
    const requested = String(worldId || '');
    const memberships = Array.isArray(word?.privateWorldIds)
      ? word.privateWorldIds.map(String)
      : [];
    const ownWorld = String(word?.customWorldId || word?.worldId || '');
    return !requested || ownWorld === requested || memberships.includes(requested);
  }

  function isEligibleForJourneyQuiz(word, gateId) {
    if (!hasLearnableContent(word)) return false;
    const requested = String(gateId || '');
    const gates = Array.isArray(word?.journeyGateIds)
      ? word.journeyGateIds.map(String)
      : [];
    return !requested || String(word?.gateId || '') === requested || gates.includes(requested);
  }
  ...
  function summarizeSources(sources) {
    ...
    const privateWorldSources = items.filter((source) => source.addedFrom === 'private-world');
    return Object.freeze({
      count: items.length,
      sources: items,
      journeySources,
      privateWorldSources,
      privateWorldCount: privateWorldSources.length,
  ...
  const API = Object.freeze({
    ...
    PERSONAL_DICTIONARY_STATES,
    normalizeIdentity,
    wordKeyOf,
    isHiddenFromDictionary,
    getPersonalDictionaryState,
    isMovedToPrivateWorld,
    isVisibleInPersonalDictionaryList,
    isVisibleInDictionaryList,
    isEligibleForPersonalDictionaryQuiz,
    isEligibleForPrivateWorldQuiz,
    isEligibleForJourneyQuiz,
