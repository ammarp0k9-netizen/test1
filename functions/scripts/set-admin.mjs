...
if (!/^[A-Za-z0-9:_-]{1,128}$/.test(uid)) {
  console.error('Usage: npm run set-admin -- <firebase-uid> [grant|revoke]');
  process.exitCode = 1;
} else {
  if (!getApps().length) {
  ...
  try {
    ...
    if (shouldGrant) claims.admin = true;
    else delete claims.admin;
    await auth.setCustomUserClaims(uid, claims);
    console.log(`Admin claim ${shouldGrant ? 'granted to' : 'revoked from'} UID ${uid}.`);
    console.log('The user must refresh their ID token or sign in again.');
  } catch (error) {
    console.error(error?.code || 'admin/claim-update-failed', error?.message || error);
    process.exitCode = 1;
  }
}
