...
test('landing is a standalone RTL page with accessible entry choices', () => {
  ...
  assert.match(landing, /data-google-sign-in/);
  assert.match(landing, /اختبار تحديد المستوى/);
  assert.match(landing, /data-app-link/);
  assert.match(landing, /الاستكشاف كضيف/);
  assert.match(landing, /aria-live="polite"/);
  assert.doesNotMatch(landing, /js\/script\.js|style\.css|tesseract|font-awesome/i);
  assert.match(landingCss, /@media \(max-width: 360px\)/);
  assert.match(landingCss, /prefers-reduced-motion: reduce/);
  assert.match(landingCss, /:focus-visible/);
  assert.doesNotMatch(landingCss, /\.header-actions \.button-quiet\s*\{\s*display:\s*none/);
});
...
test('landing reuses the production Firebase project and navigates only internally', () => {
  ...
  assert.match(landingJs, /onAuthStateChanged\(auth,/);
  assert.match(landingJs, /signInWithPopup\(auth, provider\)/);
  assert.match(landingJs, /signInWithRedirect\(auth, provider\)/);
...
test('local static server serves landing, app, privacy, assets, and app fallbacks', async (t) => {
  ...
  const cases = [
    ['/', 'text/html', /<title>LootLingua \| تعلّم الكلمات بطريقتك<\/title>/],
    ...
    ['/js/landing.js', 'text/javascript', /initializeLandingAuth/],
  ];
